...
function renderMessages(messages) {
  ...
  return messages.map(m => `<div style="margin:8px 0;padding:10px 12px;border-radius:8px;background:${m.sender === "admin" ? "#eef4ff" : "#f5f5f5"};"><strong>${m.sender === "admin" ? "Skallstuggu" : "Kunde"}</strong><br>${escapeHtml(m.message)}</div>`).join("");
}
...
async function loadRequestsWithHighlight() {
  ...
  for (const b of allBookings) {
    ...
    const overlap = checkOverlap(b), messages = await getMessages(b.id), div = document.createElement("div");
    div.className = `menu ${b.status === "approved" ? "godkjent" : b.status === "rejected" ? "avslatt" : "foresporsel"}`;
    div.innerHTML = `<p><strong>${escapeHtml(b.name || "Ukjent")}</strong></p><p>E-post: <a href="mailto:${escapeHtml(b.email)}" class="contact-link">${escapeHtml(b.email || "-")}</a><br>Telefon: <a href="tel:${escapeHtml(b.phone)}" class="contact-link">${escapeHtml(b.phone || "-")}</a></p><p>${escapeHtml(b.start_date)} → ${escapeHtml(b.end_date)}</p><p>Status: <strong>${statusText(b.status)}</strong></p>${b.message ? `<div class="customer-message"><strong>Melding fra kunde:</strong><br>${escapeHtml(b.message)}</div>` : ""}${overlap ? `<div class="warning">⚠️ Dobbelbooking med ${escapeHtml(overlap.name || "en annen forespørsel")}</div>` : ""}<div style="margin-top:14px;border-top:1px solid #ddd;padding-top:12px;"><strong>Samtale</strong>${renderMessages(messages)}</div><div style="margin:12px 0 10px;"><textarea id="reply-${b.id}" rows="3" placeholder="Skriv svar til kunden her..." style="width:100%;padding:10px;border:1px solid #ccc;border-radius:6px;resize:vertical;"></textarea><div style="margin-top:10px;"><label style="display:block;margin-bottom:6px;font-weight:600;">Legg ved faktura (PDF)</label><input type="file" id="invoice-${b.id}" accept=".pdf,application/pdf" style="width:100%;padding:8px;border:1px solid #ccc;border-radius:6px;background:white;"></div><button onclick='sendReply(${JSON.stringify(String(b.id))}, ${JSON.stringify(b.email || "")}, ${JSON.stringify(b.name || "")})' style="margin-top:10px;padding:8px 16px;background:#1976d2;color:white;border:none;border-radius:6px;cursor:pointer;">Send svar til kunde</button></div><button onclick='approve(${JSON.stringify(String(b.id))})'>Godkjenn</button><button onclick='reject(${JSON.stringify(String(b.id))})'>Avslå</button>`;
...
function checkOverlap(booking) { for (const b of allBookings) { if (b.id === booking.id || b.status !== "pending") continue; if (!(booking.end_date <= b.start_date || booking.start_date >= b.end_date)) return b; } return null; }
...
async function sendReply(bookingId, customerEmail, customerName) {
  ...
  if(!bookingId)return alert("Mangler booking-ID. Last siden på nytt og prøv igjen.");
  ...
  try {
    ...
    const response=await fetch(`${supabaseUrl}/functions/v1/resend-email`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${supabaseAnonKey}`},body:JSON.stringify({type:"admin_reply",name:customerName,email:customerEmail,message,bookingId,replyUrl})});
    if(!response.ok)throw new Error("E-posten kunne ikke sendes");
    textarea.value=""; alert("✅ Svar sendt og lagret i samtalen!"); await loadRequestsWithHighlight();

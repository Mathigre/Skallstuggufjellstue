...
async function renderBookingCard(b) {
  const messages=await getMessages(b.id), payment=b.payment_status||"unpaid", overlap=checkOverlap(b), isApproved=b.status==="approved";
...
async function sendReply(bookingId,customerEmail,customerName) {
  ...
  try {const {error:messageError}=await supabaseClient.from("booking_messages").insert({booking_id:bookingId,sender:"admin",message});if(messageError)throw messageError;const response=await fetch(`${supabaseUrl}/functions/v1/resend-email`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${supabaseAnonKey}`},body:JSON.stringify({type:"admin_reply",name:customerName,email:customerEmail,message,bookingId,replyUrl})});if(!response.ok)throw new Error("E-posten kunne ikke sendes");textarea.value="";alert("✅ Svar sendt og lagret i samtalen!");await renderDashboard();}catch(err){console.error(err);alert("Kunne ikke sende svaret: "+(err.message||err));}
}

async function approve(id) {
  const b=allBookings.find(x=>String(x.id)===String(id));if(!b)return;const overlap=checkOverlap(b);if(overlap)return alert(`❌ Dobbelbooking!\nOverlapper med ${overlap.name||"en annen booking"}`);if(!confirm(`Vil du godkjenne bookingen til ${b.name}?`))return;
  let pdfUrl=null;try{pdfUrl=await uploadInvoiceFile(id);}catch(e){console.error(e);}const {error}=await supabaseClient.from("bookings").update({status:"approved",invoice_url:pdfUrl||null,payment_status:b.payment_status||"unpaid"}).eq("id",id);if(error)return alert("Kunne ikke godkjenne: "+error.message);
  await fetch(`${supabaseUrl}/functions/v1/resend-email`,{method:"POST",headers:{"Content-Type":"application/json","Authorization":`Bearer ${supabaseAnonKey}`},body:JSON.stringify({type:"approved",name:b.name,email:b.email,phone:b.phone||"",start:b.start_date,end:b.end_date,invoiceUrl:pdfUrl||null,bookingId:String(b.id),replyUrl:`${location.origin}/reply.html?booking=${encodeURIComponent(String(b.id))}`})});alert("✅ Booking godkjent!");await loadData();
}

...
export function makeHandler({env,request=fetch}) {return async req=>{
 const reply=(body,status=200)=>Response.json(body,{status,headers:cors});
 if(req.method==='OPTIONS')return new Response('ok',{headers:cors});
 if(req.method!=='POST')return reply({error:'Kun POST er tillatt.'},405);
 try { await requireAdmin(req,env,request); } catch { return reply({error:'Admininnlogging kreves.'},403); }
 try {
